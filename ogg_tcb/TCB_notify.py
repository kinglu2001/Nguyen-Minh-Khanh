...
full_path_dirdat = input('Full path of dirdat: ').strip().rstrip('/')
...
start_count = int(input("Condition start from: "))
step = int(input("Step: "))
...
with open(f'{prefix_ext_name}_ext_prm.txt', 'w') as file:
    for i in range(1, int(num_of_thread)+1):
        ...
        file.write(
            ...
            f'EXTRACT {ext_name}\n'
            f'EXTFILE ./dirdat/{list_trail[i-1]}, MEGABYTES 1024, MAXFILES 999\n'
            f'USERID {user_source}@{sid}, PASSWORD {password}\n'
            f'REPORTCOUNT EVERY 1 SECONDS, RATE\n'
            f'TABLE {schema_source}.{table_source}, SQLPREDICATE "AS OF SCN {scn_number} WHERE {condition_column} >= {start_count} AND {condition_column} < {end_count}";\n'
            'EOF\n'
        )
...
if rep_method == 'coordinated':
    prefix_rep_name = input('Replicat prefix name (only 3 characters): ').strip().upper()
    ...
    with open(filename, 'w') as file:
        for i in range(1, int(num_of_thread) + 1):
            rep_name = prefix_rep_name + str("{:02d}".format(i))
            file.write(
